...
function getModCountBySecondaryStat(
  player: ParsedPlayer,
  targetStat: ModSecondaryStat,
  ...
  const match = player.mods.filter((mod) =>
    mod.secondary_stats.some(
      (stat) =>
        stat.name === targetStat &&
        stat.value >= lowerThreshold &&
        stat.value < upperThreshold
    )
  );
  return match.length;
}
...
function getModCountByFields(player: ParsedPlayer, matchers: ModStatMatch[]) {
  const match = player.mods.filter((mod) =>
    matchers.every(
      ([targetField, targetValue]) => mod[targetField] === targetValue
    )
  );
  return match.length;
}
...
function getDatacronCountByTier(
  player: ParsedPlayer,
  upper: number,
  lower = 1
) {
  const match = player.datacrons.filter(
    ({ tier }) => tier >= lower && tier < upper
  );
  return match.length;
}
...
const PlayerDetails = async ({ allyCode }: { allyCode: string }) => {
  ...
  return (
    <div key={player.ally_code} className="player-details">
      <label>Ally Code</label>
      <a
        className="profile-link"
        href={`https://swgoh.gg${player.url}`}
        target="_blank"
        rel="noreferrer"
      >
        {formatAllyCode(player.ally_code)}
        <NewTab />
      </a>
      ...
      <div className="gl-grid">
        <table>
          ...
          <tbody>
            <tr>
              {GLs.map((gl) => {
                const hasThisGl = ownedGLs.find(
                  (ownedGl) => ownedGl.base_id === gl.baseId
                );
                const hasUltimate = hasThisGl?.has_ultimate ?? false;
                const gearLevel =
                  hasThisGl !== undefined ? getGearOrRelicLevel(hasThisGl) : "";

                return (
                  <td key={gl.baseId} className={hasUltimate ? "ultimate" : ""}>
                    {gearLevel}
                  </td>
                );
              })}
              {GLShips.map((gl) => {
                const hasThisShip = player.units.find(
                  (unit) => unit.base_id === gl.baseId
                );
                if (!hasThisShip) return <td key={gl.baseId}></td>;

                const hasThisGl = player.units.find(
                  (unit) => unit.base_id === gl.lookupUnit
                );
      ...
      {getTWOmicronCount(player)}
      <label>Speed Mods</label>
      <div className="mods">
        <div>
          <label>25+</label>
          {getModCountBySecondaryStat(player, "Speed", 25_0000, 99_999_999)}
        </div>
        <div>
          <label>20+</label>
          {getModCountBySecondaryStat(player, "Speed", 20_0000, 25_0000)}
        </div>
        <div>
          <label>15+</label>
          {getModCountBySecondaryStat(player, "Speed", 15_0000, 20_0000)}
        </div>
      </div>
      ...
      <div className="mods">
        <div>
          <label>9</label>
          {getDatacronCountByTier(player, 10, 9)}
        </div>
        <div>
          <label>6</label>
          {getDatacronCountByTier(player, 9, 6)}
        </div>
        <div>
          <label>3</label>
          {getDatacronCountByTier(player, 6, 3)}
        </div>
        <div>
          <label>1</label>
          {getDatacronCountByTier(player, 3)}
        </div>
      </div>
    </div>
  );
